...
$(function() {
	...
	sidebar.on("click", ".chan, button", function() {
		...
		//edit disini ridwan
		var currentChannel = self.find('span.name');
		if (currentChannel.hasClass('joinpart')) {
			self.find('span.name').removeClass('joinpart');
		} else if (currentChannel.hasClass('mode')) {
			self.find('span.name').removeClass('mode');
		}
	...
	chat.on("msg", ".messages", function(e, target, msg) {
		...
		//edit disini ridwan
		if (['join','part'].indexOf(msg.type) > -1 && !button.find("span.name").hasClass("joinpart")) {
			button.find("span.name").removeClass('mode').addClass('joinpart');
		}
		if (['kick','mode'].indexOf(msg.type) > -1 && !button.find("span.name").hasClass("mode")) {
			button.find("span.name").removeClass('joinpart').addClass('mode');
		}
